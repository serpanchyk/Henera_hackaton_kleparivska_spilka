#!/usr/bin/env python3
"""
SDK-based swarm demo using go_to with body_frame=True: each drone flies 50m ahead.

Camera windows stay open after landing. Press Ctrl+C or 'q' to quit.

Run:
  source /opt/ros/humble/setup.bash
  python3 examples/swarm_goto.py
"""
...
async def drone_mission(drone: Drone, drone_id: int, takeoff_delay: float,
    ...
    try:
        ...
        waypoints = [
            ...
            (10.0, -10.0, '10m forward, 10m left'),
            (10.0, 10.0, '10m forward, 10m right'),
            (10.0, 0.0, '10m forward'),
        ]
        for n, e, label in waypoints:
            if stop_event.is_set() or shutdown.is_set():
                break
            print(f'Drone {drone_id}: {label}')
            await drone.go_to(n, e, 0.0, body_frame=True)
...
async def main():
    ...
    missions = [asyncio.create_task(
        drone_mission(d, i, 0.0, stop_event, shutdown_async))
