...
- `publish_led(value)`
  - Публікує сирі LED-команди на кшталт `1111`, `1100`, `OFF` або `BLINK`
- `spin_once()`
...
- Бінарні маски на кшталт `1111`, `1110`, `1101` та `1100`
- `ON`
- `OFF`
- `BLINK`

Поточна модель використовує фіксований PYGR-маркер: перший біт керує фіолетовим target-маячком, другий біт керує жовтим distance-маячком, третій біт керує зеленим command-маячком, четвертий біт керує червоним command-маячком. Основні стани: `1111` = FOLLOW, `1110` = HOLD, `1101` = FINISH, `1100` = SAFE.
