...
- `publish_led(value)`
  - Публікує сирі LED-команди на кшталт `1100`, `0100`, `OFF` або `BLINK`
- `spin_once()`
...
Підтримувані формати команд:

- Бінарні маски на кшталт `1100`, `1000`, `0100` та `0000`
- `ON`
- `OFF`
- `BLINK`

У поточній дво-LED моделі перший біт керує фіксованим зеленим LED `led_lens_01`, другий біт керує фіксованим червоним LED `led_lens_04`, а третій і четвертий біти не використовуються. Протокол станів: `FOLLOW` = `1100` (зелений + червоний), `HOLD` = `1000` (тільки зелений), `SAFE` = `0100` (тільки червоний), `FINISH` = синхронне блимання `1100`/`0000` приблизно 1 раз на секунду.
